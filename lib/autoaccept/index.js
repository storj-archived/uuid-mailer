...
module.exports = function autoAccept (html, cb) {
  ...
  var url = null;
  cheerio.load(html)('a').toArray().forEach(function(v) {
    if(v.attribs.href.indexOf('activations') !== -1) { url = v.attribs.href; }
  });
  ...
  request.get(url, function(e) {
    // Return the url we called for logging
    return cb(e, url);
  });
};
